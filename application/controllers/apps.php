...
class Apps extends CI_Controller {
    ...
    public function buku() {
        ...
        $a['data'] = $this->db->query("SELECT t_buku.*, r_lokasi.nama AS nm_lokasi FROM t_buku INNER JOIN r_lokasi ON t_buku.id_lokasi = r_lokasi.id LIMIT $awal, $akhir ")->result();
        $a['page'] = "d_buku";
        ...
        if ($mau_ke == "del") {
            ...
            $this->db->query("INSERT INTO t_buku VALUES ('$idp', '$id_jenis', '$judul', '$pengarang', '$penerbit', '$th_terbit', '$jml_hal', '$id_lokasi', '$stat', 'R', '$deskripsi', NOW())");

            $this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been added</div>");
            redirect('apps/buku');
        } else if ($mau_ke == "act_edt") {
            $this->db->query("UPDATE t_buku SET  id_jenis = '$id_jenis', judul = '$judul', pengarang = '$pengarang', penerbit = '$penerbit', th_terbit = '$th_terbit', jml_hal = '$jml_hal',  id_lokasi = '$id_lokasi', stat = '$stat', deskripsi = '$deskripsi'  WHERE id = '$idp'");
    ...
    public function anggota() {
        ...
        $id_a = addslashes($this->input->post('id_anggota'));
        ...
        if ($mau_ke == "del") {
            ...
            $this->db->query("INSERT INTO t_anggota VALUES ('','$id_a', '$nama', '$alamat', '$jk', '$agama', '$tmp_lahir', '$tgl_lahir', '',  NOW(), '$jenis', '1')");

            $this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been added</div>");
            redirect('apps/anggota');
        } else if ($mau_ke == "act_edt") {
            $this->db->query("UPDATE t_anggota SET id_anggota ='$id_a', nama = '$nama', alamat = '$alamat', jk = '$jk', agama = '$agama', tmp_lahir = '$tmp_lahir', tgl_lahir = '$tgl_lahir', jenis = '$jenis', stat = '$status' WHERE id = '$idp'");

            $this->session->set_flashdata("k", "<div class=\"alert alert-success\">Data has been updated</div>");
            redirect('apps/anggota');
        } else if ($mau_ke == "list_pinjam") {
            $a['id_anggota'] = $idu;
            $a['data'] = $this->db->query("SELECT * FROM t_trans WHERE id_anggota = '$idu' ORDER BY id DESC")->result();
            $a['page'] = "d_lis_pinjam";
        } else {
            $a['page'] = "d_anggota";
        }
    ...
    public function trans() {
        ...
        $cari = addslashes($this->input->post('q'));
        //view tampilan website\
        $a['data'] = $this->db->query("SELECT t_trans.*, t_anggota.nama, COUNT(t_trans.id_anggota) AS jml_pinjam 
											FROM t_trans
											LEFT JOIN t_anggota ON t_trans.id_anggota = t_anggota.id
											WHERE t_trans.stat = 'P' GROUP BY t_trans.id_anggota DESC")->result();
        $a['page'] = "d_trans";
        ...
        if ($mau_ke == "pilih_anggota") {
            ...
            $a['data'] = $this->db->query("SELECT t_trans.*, DATEDIFF(NOW(), t_trans.tgl_kembali) AS terlambat, t_buku.judul
												FROM t_trans 
												LEFT JOIN t_buku ON t_trans.id_buku = t_buku.id
												WHERE t_trans.id_anggota = '$idu' AND t_trans.stat = 'P'")->result();
    ...
    public function stat() {
        ...
        $a['buku_paling_banyak_dipinjam'] = $this->db->query("SELECT t_trans.id_buku, COUNT(t_trans.id_buku) AS jumlah_pinjam, t_buku.judul FROM t_trans LEFT JOIN t_buku ON t_trans.id_buku = t_buku.id GROUP BY t_trans.id_buku ORDER BY jumlah_pinjam DESC")->result();
        $a['anggota_paling_banyak_meminjam'] = $this->db->query("SELECT t_trans.id_anggota, COUNT(t_trans.id_anggota) AS jumlah_pinjam, t_anggota.nama FROM t_trans LEFT JOIN t_anggota ON t_trans.id_anggota = t_anggota.id GROUP BY t_trans.id_anggota ORDER BY jumlah_pinjam DESC")->result();
    ...
    public function l_peminjam() {
        ...
        $m['page'] = "l_peminjaman";
        $m['p_hariini'] = $this->db->query("SELECT t_trans.*, t_anggota.nama, t_buku.judul FROM t_trans LEFT JOIN t_anggota ON t_trans.id_anggota = t_anggota.id LEFT JOIN t_buku ON t_trans.id_buku = t_buku.id WHERE t_trans.tgl_pinjam = LEFT(NOW(), 10)")->result();
        $m['p_bulnini'] = $this->db->query("SELECT t_trans.*, t_anggota.nama, t_buku.judul FROM t_trans LEFT JOIN t_anggota ON t_trans.id_anggota = t_anggota.id LEFT JOIN t_buku ON t_trans.id_buku = t_buku.id WHERE MID(t_trans.tgl_pinjam, 6, 2) = MONTH(NOW())")->result();
        $this->load->view('admin/aaa', $m);
    }
    ...
    public function l_buku() {
        ...
        $m['page'] = "l_buku";
        $m['data'] = $this->db->query("SELECT t_buku.*, r_lokasi.nama FROM t_buku LEFT JOIN r_lokasi ON t_buku.id_lokasi = r_lokasi.id")->result();
        $this->load->view('admin/aaa', $m);
    }
    ...
    public function jq_cari_kode() {
        ...
        $q_data = $this->db->query("SELECT * FROM t_buku WHERE (judul LIKE '%$kata_kunci%' OR pengarang LIKE '%$kata_kunci%') ORDER BY id DESC");
        $data = $q_data->result();
